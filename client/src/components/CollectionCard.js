      ...
      <Card sx={{ maxWidth: "300px", width: findWidth(), borderRadius: "15px", margin:"auto" }} key={card.id} >
        ...
        >
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", width: "100%", height: "100%" }} >
            {props.size !== "xs" &&
              <IconButton onClick={flipCard} sx={{ background: "rgba(255,255,255,0.75)" }} >
                <ArrowBackIosIcon sx={{ fontSize: findButtonSize() }} />
              </IconButton>}
            <div onClick={handleOpen} style={{ height: "100%", width: "100%" }} />
            {props.size !== "xs" && <IconButton onClick={flipCard} sx={{ background: "rgba(255,255,255,0.75)" }} >
        ...
        {show &&
          ...
          >
            <Box sx={style}>
              ...
              {personal && <Button style={{borderRadius: '40px', margin: '0px 10px'}} startIcon={<Icon>settings</Icon>} variant="contained" color="primary" onClick={(e) => editCard(e, `/profile/edit_card/${card.id}`)} >Edit this collectible</Button>}
            </Box>
          </Modal>}
