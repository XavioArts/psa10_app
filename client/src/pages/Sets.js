...
const Sets = () => {
    ...
    const searchCards = async (e) => {
        e.preventDefault();
        try {
            let res = await axios.get(`/api/cards/search/${search}`);
            setCards(res.data);
        } catch (err) {
    ...
    return (
        <div>
            ...
            <hr/>
            <Box sx={{width: "80vw", bgcolor: "#D7D7D7", display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", margin: "auto", padding: "20px"}} >
